---
{
  "url": "https://www.health-samurai.io/articles/agentic-fhir-implementation-guide-development",
...
- **Write FSH files** - Define resources in FHIR Shorthand syntax
- **Run SUSHI** - Compile FSH files into FHIR JSON resources
- **Fix compilation errors** - Debug syntax issues and constraint violations
- [**Run IG Publisher**](https://confluence.hl7.org/spaces/FHIR/pages/35718627/IG+Publisher+Documentation) - Generate HTML documentation and perform final validation
- **Iterate on errors** - Address validation failures and republish
- **Manual testing** - Verify resources work in target systems
...
- **AI generates** FHIR resources from natural language
- **Aidbox validates** the resources instantly via REST API
- **AI refines** based on validation feedback
- **Process repeats** until perfect compliance is achieved
...
- Examines the CodeSystem structure from `src/CS/CSSexoBiologico.MD`
- Validates FHIR compliance against the generated JSON in `target/CSSexoBiologico.json`
- Starts the Aidbox FHIR server if needed (`docker-compose up -d`)
- Creates or runs comprehensive test suites in test/CS/
- Reports validation results and recommendations
...
- **AI generates** markdown documentation from natural language
- **AI converts** markdown to valid FHIR JSON resources
- **AI creates** comprehensive **test suites** for all resource types
- **Aidbox validates** resources in real-time via REST API
- **AI refines** based on validation feedback
- **IG Publisher generates** final documentation
