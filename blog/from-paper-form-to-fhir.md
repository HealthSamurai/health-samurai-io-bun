---
{
  "url": "https://www.health-samurai.io/articles/from-paper-form-to-fhir",
...
- **Input Analysis: **The engine ingests user-uploaded PDFs, differentiating between digitally native formats and scanned paper documents to tailor the processing.
- **Layout and Content Extraction:** Advanced Optical Character Recognition (OCR) analyzes the visual structure and extracts text, identifies potential fields, and labels essential for understanding the form&#x27;s layout.
- **Intelligent Structuring:** Machine learning models interpret this extracted information, classifying elements into field types (text inputs, checkboxes, etc.) and mapping the visual layout to a preliminary logical form structure.
- **Semantic Interpretation and FHIR Generation:** Large Language Models (LLMs) then process this structured data, understanding the form&#x27;s instructions, logic (including conditional paths), and leveraging embedded knowledge of healthcare standards to generate the FHIR SDC Questionnaire in JSON format.
- **Verification Support:** Recognizing the importance of clinical accuracy, the engine outputs the draft FHIR Questionnaire alongside tools that facilitate essential human review, using AI to highlight areas likely requiring attention.
...
- Digitally generated PDFs, created using form technologies like AcroForms, XFA (XML Forms Architecture), and PDF/A variants.
- Scanned paper forms, saved as PDFs that contained only raster images of printed documents.
...
- **Extraneous Line Feeds:** Minor syntax glitches caused by additional newlines at the start of continuation responses.
- **Lost Context and Structure Duplication:** A more serious issue where the model lost track of the previous structure, especially if the break occurred mid-string or within nested objects. It might start a new section, duplicate fields, or mismatch brackets.
...
![From Paper Form to FHIR](https://cdn.prod.website-files.com/58ecfb695710f07cd0a35e9c/681c791d3f5ed7caf174bb92_Frame%201.png)
