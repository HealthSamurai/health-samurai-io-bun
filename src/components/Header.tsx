    ...
    <header class="relative isolate z-10 bg-white" data-signals="{openMenu: '', mobileOpen: false}">
      <nav aria-label="Global" class="mx-auto flex max-w-7xl items-center justify-between p-6 lg:px-8">
        ...
        <div class="hidden lg:flex lg:gap-x-12">
          ...
          {/* Simple links */}
          {navigation.filter(n => n.href && n.label !== "Pricing").map((item) => (
          ...
          {/* Pricing link */}
          <a href="/price" class="text-sm/6 font-semibold text-gray-900 hover:text-primary">
            Pricing
          </a>
        </div>
      ...
      {/* Company Dropdown Panel - Smaller */}
      <div
        class="absolute left-1/2 top-full -translate-x-1/2 mt-3 w-56 bg-white shadow-lg ring-1 ring-gray-900/5 rounded-xl transition-all duration-200"
            ...
            <div class="space-y-2 py-6">
              ...
              <div class="-mx-3">
                <button
                  type="button"
                  class="flex w-full items-center justify-between rounded-lg py-2 pl-3 pr-3.5 text-base/7 font-semibold text-gray-900 hover:bg-gray-50"
                  data-on:click="$openMenu = $openMenu === 'mobile-company' ? '' : 'mobile-company'"
                >
                  Company
                  <span
                    class="transition-transform duration-200"
                    data-class:rotate-180="$openMenu === 'mobile-company'"
                    dangerouslySetInnerHTML={{ __html: icons.chevron }}
                  />
                </button>
                <div
                  class="mt-2 space-y-2"
                  data-show="$openMenu === 'mobile-company'"
                  style={{ display: "none" }}
                >
                  {company.map((item) => (
                    <a
